...
object CompliancePayload {
  ...
  def apply(nino: String): CompliancePayload = CompliancePayload(
    identification = Some(ObligationIdentification.apply(nino))
  )
  implicit val format: Format[CompliancePayload] = Json.format[CompliancePayload]
}
