...
object AC100002A extends UserDetailsData {
  ...
  val latePaymentPenaltyDetails2: LPPDetails = LatePaymentPenaltyDetails.lpp1DueOrOverdue(
    ReportingPeriod(2024, None),
    amount = 105.00
  ).withTimeToPay(Some(TimeToPay(TTPProposalDate = None, TTPAgreementDate = Some(LocalDate.of(2025, 2, 20)))))

  val latePaymentPenaltyDetails3: LPPDetails = LatePaymentPenaltyDetails.lpp1DueOrOverdue(
    ReportingPeriod(2024, None),
    amount = 15
  ).withSupplementary(supplement = Some(true)).withTimeToPay(Some(TimeToPay(TTPProposalDate = None, TTPAgreementDate = Some(LocalDate.of(2025, 2, 20)))))
  ...
  override val nino: String = "AC100002A"
  override val mtdItId: String = "10000"
  override val utr: String = "1000010000"
  override val description: String = "3 LPPs - TTP Agreed LPP1 (1 LPP2 ESTIMATE, 1 LPP1 DUE, 1 LPP1 DUE) with supplementary charge"
  override val descriptionOverdue: Option[String] = Some("3 LPPs - TTP Agreed LPP1 (1 LPP2 ESTIMATE, 1 LPP1 OVERDUE, 1 LPP1 OVERDUE) with supplementary charge")
